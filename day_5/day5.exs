defmodule Day5 do
  ...
  def parse(input) do
    {graph, updates} =
      ...
      |> IO.inspect(label: "DEBUG_FIRST")

    correct_updates = get_correct_updates(graph, updates)
    sum_correct_updates(correct_updates) |> IO.inspect(label: "DEBUG")
  end
  ...
  defp get_correct_updates(graph, updates) do
    Enum.filter(updates, fn update ->
      ...
      |> Enum.reduce_while(true, fn [parent, child], _ ->
        curr_list = Map.get(graph, parent) |> IO.inspect(label: "DEBUG")
  ...
  defp sum_correct_updates(list) do
    list
    |> IO.inspect(label: "DEBUG")
    |> Enum.map(fn l ->
